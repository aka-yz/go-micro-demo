...
import (
	...
	"github.com/aka-yz/go-micro-core/configs/middleware/db"
	"github.com/gocraft/dbr/v2"
	"go-micro-demo/internal/common/request"
	"go-micro-demo/internal/common/response"
	"go-micro-demo/internal/common/util"
	"go-micro-demo/internal/data/entity"
)
...
func (u *UserInfoModelImpl) UpdateUserInfo(ctx context.Context, userInfo *request.UserInfoEdit, editUser int64) (err error) {
	...
	rs, err := session.Update(entity.UserInfoTableName()).
		SetMap(userEntity.UpdateInfo).
		Exec()
	if err != nil {
		log.Error(ctx, err.Error())
		return
...
func (u *UserInfoModelImpl) GetUserInfoList(ctx context.Context, page uint64, pageSize uint64) (
	userListRes []*response.UserList, total uint64, err error) {
	log.Infof(ctx, "[GetUserInfoList] param in:%v, %v", page, pageSize)
	defer func() {
		// todo：这里其实可以进行监控上报
		// .....
	}()
	...
	num := 0
	_, err = session.Select("count(*)").From(entity.UserInfoTableName()).Load(num)
	if err != nil {
		log.Error(ctx, err.Error())
		return
	}
	...
	_, err = session.Select("*").From(entity.UserInfoTableName()).Offset(offset).Limit(pageSize).Load(userList)
	if err != nil {
		log.Error(ctx, err.Error())
		return
	}
